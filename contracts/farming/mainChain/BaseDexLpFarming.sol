...
contract BaseDexLpFarming is NonblockingLzApp {
    ...
    uint16 internal constant MESSAGE = 1;
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override {
        ...
        assembly {
            ...
            addr := mload(add(_srcAddress,20))
        }
        require(packetType == MESSAGE, "DexLpFarming: invalid message");

        (, address _account, uint256 _amount) = abi.decode(_payload, (uint16, address, uint256));

        REWARD_TOKEN.safeTransfer(_account, _amount);

        {
            bytes memory path = trustedRemoteLookup[_srcChainId];
            emit ReceivePacket(_account, path, 1, _srcChainId, _nonce);
        }
    }
