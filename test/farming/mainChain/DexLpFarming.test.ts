...
describe("DerivedDexLpFarmingMainChain", function () {
  before(async function () {
    await prepare(this, [
      "DerivedDexLpFarming",
      "DerivedDexLpFarmingLightChain",
      "MockToken",
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      [
        "chefLightChain",
        this.DerivedDexLpFarmingLightChain,
        [
    ...
    await deploy(this, [
      ...
      await this.chefLightChain.setDstChainId( dstChainId);
      await this.chefLightChain.setTrustedRemote( dstChainId, this.chef.address);
      await this.chefLightChain.setMinDstGas(dstChainId, 1 , parseUnits("1",18));
      await this.chef.setTrustedRemote( srcChainId, this.chefLightChain.address);
      await this.lzEndPoint.setDestLzEndpoint( this.chef.address,  this.lzEndPointDst.address);
      await this.lzEndPointDst.setDestLzEndpoint( this.chefLightChain.address,  this.lzEndPoint.address);
    ...
    await this.tokenTracker.safeMint(owner.address, getBigNumber(11));
    await this.tokenTracker.approve(this.chefLightChain.address, getBigNumber(11));
    await this.chefLightChain.setRewardPerBlock(
      "10000000000000000",
      parseUnits("403", 18)
    );
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of rewardToken and reward", async function () {
      ...
      expect(await this.rewardToken.balanceOf(owner.address)).to.equal(expectedrewardToken);
    });
  ...
  describe("Withdraw and Harvest", function () {
    it("Should transfer and reward and deposited token", async function () {
      ...
      expectedrewardToken = expectedrewardToken
        ...
        expect(await this.rewardToken.balanceOf(owner.address)).to.equal(beforeBalance.add(expectedrewardToken));
  ...
  describe("Light Chain reward distribution", function () {
    it("Should give reward to light chain user", async function () {
      this.nativeToken.mint(addr1.address, parseUnits("10000", 18));
      await this.tokenTracker.approve(this.chefLightChain.address, getBigNumber(1));
      let log = await this.chefLightChain.deposit(getBigNumber(11));
      expect(await this.tokenTracker.ownerOf(getBigNumber(11))).to.equal(
        this.chefLightChain.address
      );
      let t = await time.latest();

      await time.increaseTo(t + 10);
      let precision = await this.chefLightChain.ACC_REWARD_PRECISION();
      let log2 = await this.chefLightChain.withdraw(getBigNumber(11));
      expect(await this.tokenTracker.ownerOf(getBigNumber(11))).to.equal(
        owner.address
      );
      let block2 = (await ethers.provider.getBlock(log2.blockNumber)).number;
      ...
      let liqAmount = await this.tokenTracker.positions(getBigNumber(11));
      let lpSupply = await this.nativeToken.balanceOf(addr1.address);
      expectedrewardToken = expectedrewardToken.mul(precision).div(lpSupply);
      ...
      expectedrewardToken = expectedrewardToken
        ...
        .div(precision);
      expect(
        (await this.chefLightChain.userInfo(owner.address)).rewardDebt
      ).to.be.equal("-" + expectedrewardToken);
      await this.chefLightChain.harvest(this.alice.address,{value: parseEther("0.1")});
      expect(await this.rewardToken.balanceOf(owner.address)).to.equal(expectedrewardToken);
    });
  });
