...
describe("DerivedDexLpFarmingERC1155MainChain", function () {
  before(async function () {
    await prepare(this, [
      "DerivedDexLpFarmingERC1155",
      "DerivedDexLpFarmingERC1155LightChain",
       "LBPair",
        "MockToken", 
        "LZEndpointMock1"
       ,"LZEndpointMock1"
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      [
        "chefLightChain",
        this.DerivedDexLpFarmingERC1155LightChain,
        [this.lbPair.address, this.lzEndPoint.address],
      ],
    ]);
    ...
    await deploy(this, [
      [
        ...
        [this.rewardToken.address, this.lbPair.address, this.lzEndPointDst.address],
      ],
    ]);
    ...
    await this.lbPair.approveForAll(this.chefLightChain.address, true);
  ...
  describe("Light Chain reward distribution", function () {
    it("Should give reward to light chain user", async function () {
      await this.lbPair.mint(owner.address, 11, parseUnits("1000", 18));
      await this.lbPair.approveForAll(this.chefLightChain.address, true);
      await this.lbPair.setBin(
        11,
        parseUnits("1200", 18),
      ...
      let pricision = await this.chefLightChain.ACC_REWARD_PRECISION();
      let log = await this.chefLightChain.depositBatch([11], [parseUnits("100", 18)]);
      let t = await time.latest();
      await time.increaseTo(t + 10);
      let log2 = await this.chefLightChain.withdrawBatch([11]);
      let block2 = (await ethers.provider.getBlock(log2.blockNumber)).number;
      let block = (await ethers.provider.getBlock(log.blockNumber)).number;
      let expectedrewardToken = BigNumber.from("10000000000000000").mul(
        block2 - block
      );
      let liqAmount = (await this.lbPair.getBin(11))[1];
      let lpSupply = (await this.lbPair.getReserve())[1];
