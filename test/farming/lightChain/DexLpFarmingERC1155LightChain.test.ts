...
describe("DerivedDexLpFarmingERC1155LightChain", function () {
  ...
  beforeEach(async function () {
    ...
    let mockEstimatedNativeFee = parseEther("0.001");
        ...
        await this.chef.setTrustedRemote( dstChainId, this.chefMainChain.address);
        await this.chef.setMinDstGas(dstChainId, 1 , parseUnits("1",18));
        await this.chefMainChain.setTrustedRemote( srcChainId, this.chef.address);
        await this.lzEndPoint.setDestLzEndpoint( this.chefMainChain.address,  this.lzEndPointDst.address);
        await this.lzEndPointDst.setDestLzEndpoint( this.chef.address,  this.lzEndPoint.address);

    await this.rewardToken.mint(
      this.chefMainChain.address,
      parseUnits("403", 18)
    );
  ...
  describe("Withdraw and Harvest", function () {
    it("Should transfer and reward and deposited token", async function () {
      ...
      expect(await this.rewardToken.balanceOf(this.chefMainChain.address)).to.be.equal(
        parseUnits("403", 18)
      );
