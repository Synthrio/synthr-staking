...
async function main() {
  ...
  let rewardToken = process.env.REWARD_TOKEN_POLYGON_ADDRESS;
  ...
  const dexLpFarming = await DexLpFarming.deploy(`${rewardToken}`,`${tokenTracker}`,`${liquidityPool}`,`${nativeToken}`);
